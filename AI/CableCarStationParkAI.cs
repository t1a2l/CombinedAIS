...
{
    internal class CableCarStationParkAI : CableCarStationAI
    {
        ...
        public override void CalculateUnspawnPosition(ushort buildingID, ref Building data, ref Randomizer randomizer, CitizenInfo info, ushort ignoreInstance, out Vector3 position, out Vector3 target, out Vector2 direction, out CitizenInstance.Flags specialFlags)
        {
            ...
            else if (info.m_class.m_service == ItemClass.Service.Beautification)
            {
                float num3 = Building.CalculateLocalMeshOffset(m_info, data.Length);
                if (m_info.m_enterDoors != null && m_info.m_enterDoors.Length != 0)
                {
                    int num4 = randomizer.Int32((uint)m_info.m_enterDoors.Length);
                    for (int i = 0; i < m_info.m_enterDoors.Length; i++)
                    {
                        BuildingInfo.Prop prop = m_info.m_enterDoors[num4];
                        if ((prop.m_finalProp.m_doorType & PropInfo.DoorType.HangAround) == PropInfo.DoorType.HangAround && new Randomizer((buildingID << 6) | prop.m_index).Int32(100u) < prop.m_probability && data.Length >= prop.m_requiredLength)
                        {
                            ...
                            if (data.m_eventIndex != 0)
                            {
                                ...
                                EventInfo info2 = instance.m_events.m_buffer[data.m_eventIndex].Info;
                                specialFlags = info2.m_eventAI.GetHangAroundFlags(data.m_eventIndex, ref instance.m_events.m_buffer[data.m_eventIndex], buildingID, ref position, ref direction);
                            }
                            else
                            {
                                specialFlags = CitizenInstance.Flags.HangAround;
                            ...
                            else if (m_info.m_requireHeightMap)
                            {
                        ...
                        if (++num4 == m_info.m_enterDoors.Length)
                        {
                            num4 = 0;
                        }
                    }
                }
                Vector3 offset = new(0f, 0f, num3);
